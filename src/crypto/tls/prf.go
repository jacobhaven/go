...
//as defined in RFC 5246, section 5, with generic
func prf12New(hash func() hash.Hash) func(result, secret, label, seed []byte) {
	return func(result, secret, label, seed []byte) {
		labelAndSeed := make([]byte, len(label)+len(seed))
		copy(labelAndSeed, label)
		copy(labelAndSeed[len(label):], seed)
		pHash(result, secret, labelAndSeed, hash)
	}
}
...
func prfForVersion(version uint16, hashId uint8) func(result, secret, label, seed []byte) {
	...
	case VersionTLS12:
		switch hashId {
		case hashSHA384:
			return prf12New(sha512.New384)
		default:
			return prf12New(sha256.New)
		}
	default:
